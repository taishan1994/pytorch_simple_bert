...
if __name__ == '__main__':
    ...
    model.load_pretrain(512, '../../model_hub/hfl_chinese-bert-wwm-ext/')
    model.eval()
    ...
    input_token = inputs['input_ids'].to(config.device)
    segment_ids = inputs['token_type_ids'].to(config.device)
    attention_mask = inputs['attention_mask'].to(config.device)
    print(tokens)
    with torch.no_grad():
        outputs = model(input_token, segment_ids, attention_mask)
        print(outputs.shape)
        # outputs = outputs[0, 2, :]
        # print(outputs.shape)
        #
